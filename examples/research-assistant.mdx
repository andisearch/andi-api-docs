...
def research(queries: list[str], depth: str = "deep", limit: int = 10) -> dict:
...
async function research(queries, depth = "deep", limit = 10) {
  const allResults = [];
...
2. **Deep search** — `depth=deep` enables spell correction and extended source coverage
