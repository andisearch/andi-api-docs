...
Fast mode returns results in about 1 second, making it the right choice for real-time applications and high-volume workloads.
...
When you omit `searchMode`, the API defaults to `auto` — which may select fast mode for simple queries. You can also use the legacy `depth=fast` parameter. See [search modes](/search/search-modes) for the full set.
