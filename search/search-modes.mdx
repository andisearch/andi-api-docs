...
When you omit `searchMode` (or set `searchMode=auto`), Andi decides per query how much effort the search needs. Simple lookups and navigational queries resolve fast. Complex, multi-faceted, or research questions get deeper treatment — more search angles, stronger reranking, spell correction. Multi-query requests get deep coverage automatically.
