...
| `correctedQuery` | string | No | Spell-corrected query (deep mode only, when a correction is made) |
...
| `type` | string | No | Result type when classified (see [result types](#result-types)) |
...
| `cache_age_seconds` | number | How old the cached response is, in seconds. Only present on cache hits. |
...
| `search_mode` | When available | Search mode used |
...
With `metadata=full`, each article's frontmatter adds `domain`, `publisher`, `type`, `content_type`, `lang`, `word_count`, `image`, `keywords`, and `summary` when available.
