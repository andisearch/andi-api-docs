...
| `searchMode` | string | `auto` | Controls search depth and cost: `auto`, `fast`, `low-cost`, `deep`, or `exhaustive`. See [search modes](/search/search-modes). |
| `depth` | string | `fast` | Legacy alias: `fast` or `deep`. Use `searchMode` instead. When both are present, `searchMode` wins. Bare `depth` (without `searchMode`) uses flat-rate billing. |
...
| `units` | string | auto | Unit system for weather/calculations: `metric`, `imperial`. Defaults based on `country`. |
...
curl "https://api.andiai.com/api/v1/search?q=AI+news&dateRange=week" \
  -H "x-api-key: YOUR_API_KEY"
```
...
Location can be passed as flat keys on GET or POST. POST is preferred for location-bearing requests.
...
<CardGroup cols={2}>
  <Card title="Search modes" icon="gauge-high" href="/search/search-modes">
    How searchMode controls speed, depth, and cost.
  </Card>
