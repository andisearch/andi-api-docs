...
A `401` for a missing key includes a machine-readable `signup` field so an agent can self-serve a key without a human in the loop:
...
```json
{
  ...
  "signup": {
    "trialKeyUrl": "https://console.andiai.com/signup",
    "docsUrl": "https://docs.andiai.com"
  }
}
```
</Accordion>
...
A `402` includes a machine-readable `upgrade` field an agent can use to point its user at the right place:
...
```json
{
  ...
  "message": "Your account has insufficient credits. Please add credits to continue.",
  "upgrade": {
    "addCreditsUrl": "https://console.andiai.com/billing",
    "docsUrl": "https://docs.andiai.com"
  }
}
```
