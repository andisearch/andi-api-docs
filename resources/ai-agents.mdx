...
| `searchMode` | string | `auto` | Cost/speed/coverage dial: `auto`, `fast`, `low-cost`, `balanced`, `deep`, or `exhaustive`. `fast` ~1s lowest latency; `balanced` everyday web search; `deep` ~2–3s adds spell correction and more engines; `exhaustive` multi-round agentic retrieval (up to ~15s). |
